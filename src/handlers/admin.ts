...
const ADM_SECTION_PREFIX = "adm_sec:";
const ADM_SECTION_EDIT_PREFIX = "adm_sec_edit:";
...
const ADM_DEL_SECTION_PREFIX = "adm_sec_del:";
const ADM_CONFIRM_DEL_SECTION_PREFIX = "adm_sec_confirm_del:";
const ADM_SECTION_SUB_PREFIX = "adm_sec_sub:";
const ADM_DEL_SECTION_SUB_PREFIX = "adm_sec_del_sub:";
const ADM_CONFIRM_DEL_SECTION_SUB_PREFIX = "adm_sec_confirm_del_sub:";
...
const ADM_RESTORE_SECTION_PREFIX = "adm_restore_sec:";
...
function getAdminRestoreMenu() {
  ...
  hiddenSections.forEach((id) => {
    ...
    buttons.push([Markup.button.callback(`📌 ${label} (главное меню)`, `${ADM_RESTORE_SECTION_PREFIX}${id}`)]);
  });
...
export function registerAdmin(bot: {
  ...
  bot.action(new RegExp(`^${ADM_RESTORE_SECTION_PREFIX}`), async (ctx) => {
    if (!canUseAdmin(ctx)) {
      await ctx.answerCbQuery();
      return;
    }
    ...
    if (!data) return;
    const id = data.slice(ADM_RESTORE_SECTION_PREFIX.length);
    const label = getSectionLabel(id);
    await withErrorHandling(ctx, async () => {
      await ctx.answerCbQuery();
  ...
  bot.action(new RegExp(`^${ADM_SECTION_PREFIX}[^:]+$`), async (ctx) => {
    if (!canUseAdmin(ctx)) {
      await ctx.answerCbQuery();
      return;
    }
    ...
    await withErrorHandling(ctx, async () => {
      await ctx.answerCbQuery();
      const buttons: ReturnType<typeof Markup.button.callback>[][] = [
        [Markup.button.callback("✏️ Редактировать название", `${ADM_SECTION_EDIT_PREFIX}${key}`)],
        hidden
  ...
  bot.action(new RegExp(`^${ADM_SECTION_EDIT_PREFIX}`), async (ctx) => {
    if (!canUseAdmin(ctx)) {
      await ctx.answerCbQuery();
      return;
    }
    ...
    const data = cq && "data" in cq ? cq.data : undefined;
    if (!data) return;
    const key = data.slice(ADM_SECTION_EDIT_PREFIX.length);
  ...
  bot.action(new RegExp(`^${ADM_DEL_SECTION_PREFIX}`), async (ctx) => {
    if (!canUseAdmin(ctx)) {
      await ctx.answerCbQuery();
      return;
    }
    ...
    const data = cq && "data" in cq ? cq.data : undefined;
    if (!data) return;
    const id = data.slice(ADM_DEL_SECTION_PREFIX.length);
    const label = getSectionLabel(id);
    await withErrorHandling(ctx, async () => {
      await ctx.answerCbQuery();
      await ctx.editMessageText(
        `Удалить пункт «${label}» из главного меню? Пользователи больше не увидят его в меню.`,
        {
          ...
          ...Markup.inlineKeyboard([
            [Markup.button.callback("Да, удаляем", `${ADM_CONFIRM_DEL_SECTION_PREFIX}${id}`)],
  ...
  bot.action(new RegExp(`^${ADM_CONFIRM_DEL_SECTION_PREFIX}`), async (ctx) => {
    if (!canUseAdmin(ctx)) {
      await ctx.answerCbQuery();
      return;
    }
    ...
    const data = cq && "data" in cq ? cq.data : undefined;
    if (!data) return;
    const id = data.slice(ADM_CONFIRM_DEL_SECTION_PREFIX.length);
    const label = getSectionLabel(id);
    await withErrorHandling(ctx, async () => {
      await ctx.answerCbQuery();
  ...
  bot.action(new RegExp(`^${ADM_SECTION_SUB_PREFIX}list:`), async (ctx) => {
    if (!canUseAdmin(ctx)) {
      await ctx.answerCbQuery();
      return;
    }
    ...
    const data = cq && "data" in cq ? cq.data : undefined;
    if (!data) return;
    const sectionKey = data.slice(`${ADM_SECTION_SUB_PREFIX}list:`.length);
    const sections = getSections();
    ...
    await withErrorHandling(ctx, async () => {
      ...
      const subButtons = subIds.map((itemKey) => {
        ...
        return [Markup.button.callback(label, `${ADM_SECTION_SUB_PREFIX}${sectionKey}:${itemKey}`)];
      });
      const keyboard = [
        ...subButtons,
        [Markup.button.callback("◀️ Назад", `${ADM_SECTION_PREFIX}${sectionKey}`)],
      ];
  ...
  bot.action(new RegExp(`^${ADM_SECTION_SUB_PREFIX}[^l]`), async (ctx) => {
    if (!canUseAdmin(ctx)) {
      await ctx.answerCbQuery();
      return;
    }
    ...
    const data = cq && "data" in cq ? cq.data : undefined;
    if (!data) return;
    const rest = data.slice(ADM_SECTION_SUB_PREFIX.length);
    ...
    await withErrorHandling(ctx, async () => {
      ...
      await ctx.editMessageText(`Подпункт «${item.label}». Что сделать?`, {
        parse_mode: "HTML",
        ...Markup.inlineKeyboard([
          [Markup.button.callback("🗑 Удалить подпункт", `${ADM_DEL_SECTION_SUB_PREFIX}${sectionKey}:${itemKey}`)],
          [Markup.button.callback("◀️ Назад", `${ADM_SECTION_SUB_PREFIX}list:${sectionKey}`)],
        ]),
      });
    });
  });

  bot.action(new RegExp(`^${ADM_DEL_SECTION_SUB_PREFIX}`), async (ctx) => {
    if (!canUseAdmin(ctx)) {
      await ctx.answerCbQuery();
      return;
    }
    ...
    const data = cq && "data" in cq ? cq.data : undefined;
    if (!data) return;
    const rest = data.slice(ADM_DEL_SECTION_SUB_PREFIX.length);
    const colon = rest.indexOf(":");
    if (colon === -1) return;
    const sectionKey = rest.slice(0, colon);
    const itemKey = rest.slice(colon + 1);
    ...
    await withErrorHandling(ctx, async () => {
      ...
      await ctx.editMessageText(
        `Удалить подпункт «${item.label}»?`,
        {
          parse_mode: "HTML",
          ...Markup.inlineKeyboard([
            [Markup.button.callback("Да, удаляем", `${ADM_CONFIRM_DEL_SECTION_SUB_PREFIX}${sectionKey}:${itemKey}`)],
            [Markup.button.callback("Нет, оставляем", `${ADM_SECTION_SUB_PREFIX}${sectionKey}:${itemKey}`)],
          ]),
        }
      );
    });
  ...
  bot.action(new RegExp(`^${ADM_CONFIRM_DEL_SECTION_SUB_PREFIX}`), async (ctx) => {
    if (!canUseAdmin(ctx)) {
      await ctx.answerCbQuery();
      return;
    }
    ...
    const data = cq && "data" in cq ? cq.data : undefined;
    if (!data) return;
    const rest = data.slice(ADM_CONFIRM_DEL_SECTION_SUB_PREFIX.length);
    const colon = rest.indexOf(":");
    if (colon === -1) return;
    const sectionKey = rest.slice(0, colon);
    const itemKey = rest.slice(colon + 1);
    ...
    await withErrorHandling(ctx, async () => {
      ...
      await ctx.editMessageText(`Подпункт удалён.`, {
        parse_mode: "HTML",
        ...Markup.inlineKeyboard([[Markup.button.callback("◀️ К подпунктам", `${ADM_SECTION_SUB_PREFIX}list:${sectionKey}`)]]),
      });
    });
  });
